/*
 * HISTORY
 * 27-Jan-89  Peter King (king) at NeXT
 *	NFS 4.0 Changes.
 *
...
#endif
#if	MACH
	xprt = (SVCXPRT *)kalloc((u_int)sizeof(SVCXPRT));
	rpc_buffer(xprt) = (caddr_t)kalloc((u_int)UDPMSGSIZE);
	ud = (struct udp_data *)kalloc((u_int)sizeof(struct udp_data));
#else	MACH
...
#if	MACH
	kfree((caddr_t)ud, (u_int)sizeof(struct udp_data));
	kfree((caddr_t)rpc_buffer(xprt), (u_int)UDPMSGSIZE);
	kfree((caddr_t)xprt, (u_int)sizeof(SVCXPRT));
#else	MACH
...
#if	MACH
		dr = (struct dupreq *)kalloc(sizeof(*dr));
#else	MACH
...
#if	NNFSMEAS
svckudp_xid(struct svc_req *req)
{
	return (REQTOXID(req));
}
#endif	NNFSMEAS
