 ...
 *
 * HISTORY
 * 23-Mar-90  Gregg Kellogg (gk) at NeXT
 ...
 *
 *  8-Feb-88  David Black (dlb) at Carnegie-Mellon University
 *	Pass map entry to vm_fault_{wire,unwire}.
 *	Don't need recursive lock to unwire in vm_map_pageable.
 *
 * 15-Dec-87  David Golub (dbg) at Carnegie-Mellon University
 *	Check for null map pointer in vm_map_reference and
 *	vm_map_deallocate.
 *
 *  3-Dec-87  Michael Young (mwyoung) at Carnegie-Mellon University
 *	Change variable "shared" to "is_shared" in vm_region()
 *	to accomodate compilers for which it is a reserved keyword.
 *
 * 20-Nov-87  Michael Young (mwyoung) at Carnegie-Mellon University
 *	Changed interface to vm_map_lookup() to return a map version;
 *	added vm_map_verify() and vm_map_verify_done() routines.
 *	
 *	Removed old history.
 *
 *  1-Dec-87  Karl Hauth (hauth) at Carnegie-Mellon University
 *	Fixed vm_region() call to unlock the map even if we get an error.
 *
 ...
 *	Added vm_region() call; removed old print format.
 *	Added timestamps to maps.
 *
 *  6-Oct-87  Robert Baron (rvb) at Carnegie-Mellon University
 *	Added clipping to vm_map_submap().
 *
 *  5-Oct-87  David Golub (dbg) at Carnegie-Mellon University
 *	Once again, use pmap_protect in vm_map_copy_entry if source
 *	entry has only one possible mapping.
 *
 * 30-Sep-87  David Golub (dbg) at Carnegie-Mellon University
 *	Changed vm_map_pageable to hold only a read lock on the
 *	map while wiring pages.
 *
 */
...
{
	if (map->entries_pageable)
		ZFREE(vm_map_entry_zone, (vm_offset_t) entry);
	else
		ZFREE(vm_map_kentry_zone, (vm_offset_t) entry);
}
	...
	vm_offset_t	*offset_in_object;	/* OUT */
{
	...
	*size = (entry->end - start);
	...
	if (entry->is_a_map) {
		...
		*offset_in_object = tmp_entry->offset + (tmp_offset - tmp_entry->start);

		*is_shared = (share_map->ref_count != 1);
		vm_map_unlock_read(share_map);
	} else if (entry->is_sub_map) {
		*is_shared = FALSE;
		*object_name = PORT_NULL;	// FIXME: what name for submap?
