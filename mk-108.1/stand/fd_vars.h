...
struct fd_controller {
	...
	struct dma_hdr		fifo_dma_hd;	/* for enqueueing "fifo fill"
						 * DMA */
...
struct fd_volume {
	...
	u_char label_buf[FD_SECTALIGN(sizeof(struct disk_label), 
				FD_SECTSIZE_MAX) + DMA_ENDALIGNMENT-1];
	struct disk_label 	*labelp;	/* aligned ptr to disk label */
	struct fd_ioreq		dev_ioreq;	/* every I/O request is
						 * placed here when command is 
						 * passed to controller
						 * thread */
	struct iob		iob;		/* normal mapped read/write
...
#define FVIOF_SPECIAL	0x00000002		/* 1 = command in 
						 * fvp->dev_ioreq; else cmd in 
						 * fvp->iob. */
#define FVIOF_LPART	0x00000004		/* live partition; no label
						 * necessary, no bad block
						 * mapping */
 ...
 * in fd_io.c:
 */
int fc_init(caddr_t hw_reg_ptr);
...
extern struct fd_global fd_global;
...
int fdopen(struct iob *iop);
void fdclose(struct iob *iop);
int fdstrategy(struct iob *iop, int func);
