 ...
 *	Backed out NFS 4.0 READDIR changes.  We will continue to use
 *	struct direct rather than struct dirent.
 *
 * 21-Dec-88  Peter King (king) at NeXT
 *	NFS 4.0 Changes: mbuf hack.
 */
...
#define MINDIRSIZ(dp)	(sizeof(struct direct) - MAXNAMLEN + (dp)->d_namlen)
...
/* there was an error in calculating the record length of each directory */
	...
	struct nfsrddirres *rd;
{
	...
	for (;;) {
		...
		if (valid) {
			if (size < (int)(sizeof (u_long) + sizeof (u_short)) ||
			    !xdr_u_long(xdrs, &dp->d_fileno) ||
