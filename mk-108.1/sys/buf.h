 ...
 *
 * History
 * 05-Jul-90	Doug Mitchell at NeXT
 *	Added B_KERNSPACE flag.
 */
...
#ifdef	NeXT
	int	b_sort_key;		/* used by disk sort */
	int	b_rtpri;		/* realtime priority */
#define	RTPRI_NONE	0
#define	RTPRI_MIN	1
#define	RTPRI_MAX	127
#endif	NeXT
...
struct	buf *breada();
#if	NeXT
struct	buf *getemptybuf();
struct	buf *getpblk();
struct	buf *breadp();
#endif	NeXT
struct	vnode *bdevvp();	/* Added for use by the specfs routines */
...
#define	B_PHYS_COPY	0x01000000	/* phys I/O copied because unaligned */
#define B_PRIVATE	0x02000000	/* memory attached to buffer is private */
