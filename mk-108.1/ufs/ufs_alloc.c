 ...
 *	NFS 4.0 Changes.
 *	Removed SUN_VFS compiler switches.
 *
 ...
 *	CS_BUGFIX: Some machines' file systems have a random value in
 ...
 *	Don't call mfind/munhash for MACH_VM.
 *
...
#ifndef lint
/* static char sccsid[] = 	"@(#)ufs_alloc.c	2.1 88/05/20 4.0NFSSRC SMI"; */
	/* from UCB 7.1 6/5/86 */ 
	/* @(#) from SUN 2.20	*/
#endif
 ...
 *  lower level).  The caller may prohibit any pausing with a non-zero parameter
 *  (i.e. when pauses are being prohibited on a per-file basis) for the side
 *  effect of resetting the resource pause fields.
 *
    ...
    int nopause;
{
    ...
    u.u_rpsfs = 0;
    u.u_rpswhich = 0;
    if (fs == 0 || which == 0 || u.u_error != ENOSPC ||
        (u.u_rpause&URPS_NOSPC) == 0 || nopause)
	return(FALSE);

    u.u_error = 0;
    if (! rpsleep(fssleep, (int)fs, which, fs->fs_fsmnt,
...
#if	SCSI_ALLOC
	/*
	 * An optimization for SCSI disks...
	 * Because SCSI drives hide the mapping of SCSI lba's to
	 * cylinder/track/sector and don't have a simple nsectors * ntracks
	 * sectors on a cylinder, it's impossible to know what sectors
	 * are within a cylinder making the normal BSD rotational optimization
	 * algorithm difficult to implement correctly.
	 * As a second best attempt, we try to find a block that is
	 * "ahead" by multiples of a track for up to a half a cylinder forward.
	 * While this may put the block on the next cylinder, it's no
	 * worse than randomly placing the block in the cylinder group
	 * and probably better than the what results from the normal BSD
	 * algorithm when making bogus assumptions about SCSI disk geometry.
	 */
	for (i = 0; i < fs->fs_ntrak/2; i++) {
		bno = bpref + roundup(i * fs->fs_nsect / NSPF(fs), fs->fs_frag);
		if (bno + fs->fs_frag > fs->fs_fpg) {
#ifdef notdef
			bpref = 0;	/* ???? */
#endif notdef
			break;
	...
	off_t size;
{
	...
	if (size == fs->fs_bsize) {
		if (isblock(fs, cgp->cg_free, fragstoblks(fs, bno))) {
			...
			panic("free_block: freeing free block");
		}
		...
		for (i = 0; i < frags; i++) {
			if (isset(cgp->cg_free, bno + i)) {
				printf("dev = 0x%x, block = %d, fs = %s\n",
				    ip->i_dev, bno + i, fs->fs_fsmnt);
				panic("free_block: freeing free frag");
			}
