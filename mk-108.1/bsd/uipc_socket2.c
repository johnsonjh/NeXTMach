 ...
 *	SUN_VFS: Remove inode.h include.
 *	SUN_NFS: Only wake up one NFS daemon at a time if nfs_wakeup_one_nfsd
 *		 is set.
 *
...
#if	NeXT
		/*
		 * If the attach failed, a lower level might called sofree
		 * which does a soqremque(so, 0) and soqremque(so, 1)
		 * which then sets so->so_head = NULL.  Don't call
		 * soqremque if this is the case.
		 */
		if (so->so_head) {
			(void) soqremque(so, 0);
		}
	...
	struct sockbuf *sb;
{
	...
	if ((unsigned) cc > (unsigned)SB_MAX * MCLBYTES /
	    (2 * MSIZE + MCLBYTES))
		return (0);
	sb->sb_hiwat = cc;
	...
	register struct mbuf *m, *n;
{
	...
	while (m) {
		...
		/*
		 * If neither the current nor the next mbuf is a cluster
